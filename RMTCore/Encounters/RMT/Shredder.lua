...
mod:RegisterDefaultSetting("PrintSwabbieSpeed", false)
...
mod:RegisterDefaultSetting("Announce10BileStacks")
mod:RegisterDefaultSetting("SoundOozeStacksWarning")

mod:RegisterDefaultTimerBarConfigs({
    ["BUBBLES"] = { sColor = "xkcdSunYellow" },
})
...
function mod:OnCastStart(nId, sCastName, nCastEndTime, sName)
    if self.L["Noxious Nabber"] == sName then
        if self.L["Necrotic Lash"] == sCastName then
            ...
            if mod:GetDistanceBetweenUnits(player, tUnit) < 30 then
...
function mod:OnUnitCreated(nId, unit, sName)
    ...
    if sName == self.L["Swabbie Ski'Li"] then
        ...
        nSwabbieSkiLiId = nId
		saw_count = 0
		saw_color = "xkcdBrightPurple"
    elseif sName == "Noxious Nabber" then
        core:WatchUnit(unit)
...
--        line:SetMaxLengthVisible(45)
    ...
    elseif sName == self.L["Sawblade"] then
        if mod:GetSetting("DrawLargeSawLines") then
			...
			if (saw_count % 2 == 1) then 
				if (saw_color == "xkcdBrightPurple") then
					saw_color = "xkcdBloodOrange"
				else
					saw_color = "xkcdBrightPurple"
				end
			end
			if ((saw_count == 1) or (saw_count == 2)) and (bMini1Spawned) then
				core:AddPixie(nId, 2, unit, nil, saw_color, 15, 90, 0)
			else
				core:AddPixie(nId, 2, unit, nil, saw_color, 15, 45, 0)		
			end
        end
...
function mod:OnDebuffUpdate(nId, nSpellId, nStack, fTimeRemaining)
    local tUnit = GameLib.GetUnitById(nId)
    if nSpellId == DEBUFF__OOZING_BILE then
        if nStack >= 8 then
            if tUnit == GameLib.GetPlayerUnit() then
                local sMessage = self.L["%d STACKS!"]:format(nStack)
                mod:AddMsg("STACK", sMessage, 5, "Beware")
            end
        end
    end
end
